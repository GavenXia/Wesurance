<template>
  <div class="page-wrap">
    ...
    <div class="container-wrap">
      ...
      <p class="part-title">
        <span>Departure and Return Date</span>
        <span>{{days}} days</span>
      </p>
      <div class="date-part">
      ...
      <div class="button-bar">
        <!-- <router-link to="/info"> -->
        <button class="next" @click="show">Show My Results</button>
        <!-- </router-link>            -->
      </div>
    </div>
  </div>
...
export default {
  data() {
    return {
      imgs: {
        src: require("@/assets/teafInsurance/demo.png"), //starrTraveLeadEssential.png
        preview: require("@/assets/teafInsurance/demo.png")
      },
      date: {
  ...
  methods: {
    ...
    show() {
      if(!this.date.returnDate) {
        ...
        return false;
      } else if(new Date(this.date.returnDateObj).getTime() < new Date(`${this.date.departureDate} 00:00:00`).getTime()) {
         this.$createToast({
         txt: '返程日期不能小于去程日期',
         time: 3000,
         type: 'txt'
      ...
      // var areacode=this.$route.params.phone_country_code
      const areacode = "852";
      const query = {
        count: this.count,
        days:this.days,
        date: {
          departureDate: this.date.departureDate,
          returnDate: this.date.returnDate
        },
        product_id: "5d15cdc2971f320d262b8266",
        phone_country_code: areacode
      };

      this.$router.push({ path: "/Infoconfirm", query: { travelInfo:JSON.stringify(query) } });
    }
  },
  computed: {
    counts() {
    ...
    days() {
      let days = 0
      if(this.date.returnDate && (new Date(this.date.returnDateObj).getTime() >= new Date(`${this.date.departureDate} 00:00:00`).getTime())) {
       days = Math.ceil((new Date(this.date.returnDateObj).getTime() - new Date(this.date.departureDateObj))/(1000*60*60*24)) + 1
      }
      return days
    }
  }
